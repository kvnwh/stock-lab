...
for point in points:
    ...
    if prevOpen < prevClose:  # prev was green
        if (
            p.open < p.close and p.open > prevOpen and p.close > prevClose
        ):  # green going up
            print(
                f'continue up time: {p.time.astimezone().strftime("%y-%m-%d %H:%M:%S")} open:{p.open} close:{p.close} high:{p.high} low:{p.low} prevClose:{prevClose}'
            )
    elif prevOpen > prevClose:  # prev was red
        if (
            p.open < p.close
            and p.open <= prevClose
            and p.close >= (prevOpen + prevClose) / 2
        ):
            print(
                f'turning up time: {p.time.astimezone().strftime("%y-%m-%d %H:%M:%S")} open:{p.open} close:{p.close} high:{p.high} low:{p.low} prevClose:{prevClose}'
            )
    # prevHigh = p.high
    prevClose = p.close
    prevOpen = p.open
